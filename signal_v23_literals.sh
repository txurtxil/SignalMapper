...
echo "🔧 1/2 Arreglando los textos literales (String Interpolation Fix)..."
...
class DatabasePro {
  ...
  static Future<void> init() async {
    db = await openDatabase(p.join(await getDatabasesPath(), 'signal_v23.db'),
      onCreate: (db, version) {
  ...
  @override Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: Stack(children: [
        ...
        // HUD CORREGIDO SIN BARRAS INVERTIDAS
...
echo "🚀 2/2 Compilando la V23 de corrección..."
flutter build apk --profile

if [ -f "build/app/outputs/flutter-apk/app-profile.apk" ]; then
    gh release create v23.0-literals-fixed build/app/outputs/flutter-apk/app-profile.apk --repo txurtxil/SignalMapper --title "🔥 SignalMapper V23 TEXTOS ARREGLADOS" --notes "Arreglado el bug visual donde se mostraban nombres de variables en lugar de los valores numéricos."
    echo "===================================================="
    echo "✅ ¡HECHO! Todo arreglado y listo."
    echo "===================================================="
else
