...
echo "📡 1/3 Instalando sensor de decibelios (dBm)..."
flutter pub add signal_strength

echo "🧠 2/3 Inyectando lógica de mapa de calor profesional..."
...
class _SignalMapperRealState extends State<SignalMapperReal> {
  ...
  void _toggleTracking() {
    setState(() {
      ...
      if (isTracking) {
        // Registra datos cada 5 segundos para un mapa más denso
  ...
  Future<void> _recordData() async {
    try {
      // 1. Obtener intensidad de señal real (dBm)
  ...
  Color _getSignalColor(int dbm) {
    ...
    if (dbm >= -95) return Colors.yellow[700]!; // Buena/Media
    if (dbm >= -105) return Colors.orange[800]!; // Mala
    return Colors.red[900]!;                    // Muy mala / Sin señal
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text(isTracking ? "Intensidad: $currentDbm dBm" : "Auditor de Red v7"),
        ...
        children: [
          ...
          Positioned(
            ...
            child: ElevatedButton.icon(
              ...
              label: Text(isTracking ? "DETENER MAPEO" : "INICIAR AUDITORÍA dBm"),
...
echo "🚀 3/3 Compilando v7 con sensor dBm real..."
flutter build apk --release --no-pub

if [ -f "build/app/outputs/flutter-apk/app-release.apk" ]; then
    gh release create v7.0-dbm build/app/outputs/flutter-apk/app-release.apk --repo txurtxil/SignalMapper --title "🚀 SignalMapper V7 (dBm Real)" --notes "Mapeo de intensidad real. Colores dinámicos por potencia."
    echo "✅ LISTO. Descarga la v7 en GitHub y a caminar por Cruces."
else
