...
echo "🧹 1/3 Eliminando librerías conflictivas..."
flutter pub remove file_picker
flutter pub add file_selector
flutter pub get

echo "🧠 2/3 Inyectando Lógica de Importación Oficial de Google..."
...
import 'package:file_selector/file_selector.dart' as fs; // LIBRERÍA OFICIAL DE GOOGLE
...
class DatabasePro {
  ...
  static Future<void> init() async {
    db = await openDatabase(p.join(await getDatabasesPath(), 'signal_v18.db'),
      onCreate: (db, version) {
...
// ================= OUTDOOR (CON IMPORTACIÓN BLINDADA) =================
...
class _OutdoorProState extends State<OutdoorPro> {
  ...
  // IMPORTACIÓN USANDO FILE_SELECTOR
...
// ================= INDOOR (CON IMPORTACIÓN BLINDADA) =================
...
echo "🚀 3/3 Compilando v18 Bulletproof..."
flutter build apk --profile

if [ -f "build/app/outputs/flutter-apk/app-profile.apk" ]; then
    gh release create v18.0-importer build/app/outputs/flutter-apk/app-profile.apk --repo txurtxil/SignalMapper --title "🔥 SignalMapper V18 BULLETPROOF IMPORTER" --notes "Importación de CSV usando File_Selector oficial para evitar bloqueos."
    echo "===================================================="
    echo "✅ ¡COMPILADO SIN ERRORES! Listo en GitHub."
    echo "===================================================="
else
    echo "❌ Revisa la terminal, la compilación falló."
fi
