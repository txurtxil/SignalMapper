...
echo "🔧 1/3 Aplicando parche de Namespace a la librería..."
...
if [ -f "$TARGET_FILE" ]; then
    ...
    echo "✅ Parche de Namespace aplicado."
fi

echo "📝 2/3 Corrigiendo código Dart (signalStrength getter)..."
...
class _SignalMapperRealState extends State<SignalMapperReal> {
  ...
  Future<void> _recordData() async {
    try {
      // CORRECCIÓN: .signalStrength es un getter, no una función
      List<int>? signals = await _signalManager.signalStrength;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text(isTracking ? "Potencia: $currentDbm dBm" : "SignalMapper v7.3"),
...
echo "🚀 3/3 Compilando v7.3 (Código corregido)..."
flutter build apk --release --no-pub

if [ -f "build/app/outputs/flutter-apk/app-release.apk" ]; then
    gh release create v7.3-final build/app/outputs/flutter-apk/app-release.apk --repo txurtxil/SignalMapper --title "🚀 SignalMapper V7.3 (Fix API)" --notes "Corregido el error de Dart getSignalStrength vs signalStrength."
    echo "===================================================="
    echo "✅ ¡BINGO! Compilación terminada."
    echo "Descarga el APK aquí: https://github.com/txurtxil/SignalMapper/releases/latest"
    echo "===================================================="
else
    echo "❌ Fallo en la compilación. Revisa el log de arriba."
fi
