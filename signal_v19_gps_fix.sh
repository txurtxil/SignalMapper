...
echo "🛰️ 1/2 Inyectando Protocolos de Rescate GPS..."
...
class DatabasePro {
  ...
  static Future<void> init() async {
    db = await openDatabase(p.join(await getDatabasesPath(), 'signal_v19.db'),
      onCreate: (db, version) {
...
class _PowerProNavigationState extends State<PowerProNavigation> {
  ...
  Future<void> _requestPermissions() async {
    ...
    // Doble chequeo nativo de Geolocator por si Android se pone estricto
...
class _OutdoorProState extends State<OutdoorPro> {
  ...
  LatLng? currentPos; 
  // ESTADO POR DEFECTO PARA QUE EL HUD NUNCA DESAPAREZCA
  Map<String, dynamic> currentAudit = {'dbm': 0, 'tech': 'Iniciando antenas...', 'operator': '-', 'cell_id': '-'};
  ...
  void _toggleTracking() async { 
    ...
    if (!serviceEnabled) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("❌ ERROR: El GPS físico del móvil está apagado."), backgroundColor: Colors.red));
      return;
    }
    ...
    setState(() { 
      ...
      if (isTracking) {
        ...
        // Forzamos un primer registro inmediato
        _recordData();
      } else { timer?.cancel(); }
    }); 
  }
  ...
  Future<void> _recordData() async {
    try {
      ...
      // PARCHE GPS: Añadimos un tiempo límite de 3 segundos para que no se congele
      Position? p;
      try {
        p = await Geolocator.getCurrentPosition(desiredAccuracy: LocationAccuracy.high, timeLimit: const Duration(seconds: 3));
      } catch (e) {
        // Si falla por timeout, usamos la última ubicación conocida
        p = await Geolocator.getLastKnownPosition();
        if (p == null) throw Exception("No se pudo obtener ninguna ubicación GPS.");
      }
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text("⚠️ $e"), duration: const Duration(seconds: 2)));
      }
    }
  }
  ...
  @override Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Stack(children: [
        ...
        // HUD PERMANENTE (Ya no desaparece)
...
echo "🚀 2/2 Compilando v19 con Rescate GPS..."
flutter build apk --profile

if [ -f "build/app/outputs/flutter-apk/app-profile.apk" ]; then
    gh release create v19.0-gps-fix build/app/outputs/flutter-apk/app-profile.apk --repo txurtxil/SignalMapper --title "🔥 SignalMapper V19 GPS OVERDRIVE" --notes "Solucionado el bloqueo de GPS. Ahora muestra el HUD siempre y gestiona Timeouts de geolocalización."
    echo "===================================================="
    echo "✅ ¡COMPILADO! Mapeo sin cuelgues."
    echo "===================================================="
else
