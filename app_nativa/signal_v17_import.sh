...
echo "📥 1/3 Instalando Selector de Archivos (File Picker)..."
flutter pub add file_picker
flutter pub get

echo "🧠 2/3 Inyectando Lógica de Importación y Lectura de CSV..."
  ...
  Future<void> _recordData() async {
    try {
      ...
      if (mounted) setState(() {
        ...
        _points.add(CircleMarker(point: pos, color: getGodColor(audit['dbm'] ?? -120), radius: 12));
      });
    } catch (e) {}
  }

  // IMPORTAR CSV OUTDOOR
    ...
    if (result != null) {
      ...
      for (int i = 1; i < lines.length; i++) {
        ...
        if (cols.length > 8 && cols[2] == "outdoor") {
          ...
          setState(() {
            _routeLine.add(pos);
            _points.add(CircleMarker(point: pos, color: getGodColor(dbm), radius: 12));
          });
    ...
    return Scaffold(
      ...
      body: Stack(children: [
        FlutterMap(mapController: _mapController, options: MapOptions(initialCenter: LatLng(43.297, -2.985), initialZoom: 17), children: [
          ...
          PolylineLayer(polylines: [Polyline(points: _routeLine, color: Colors.blueAccent, strokeWidth: 5)]),
...
echo "🚀 3/3 Compilando V17 Data Importer..."
flutter build apk --profile

if [ -f "build/app/outputs/flutter-apk/app-profile.apk" ]; then
    gh release create v17.0-import build/app/outputs/flutter-apk/app-profile.apk --repo txurtxil/SignalMapper --title "🔥 SignalMapper V17 DATA IMPORT" --notes "Importación de archivos CSV para comparar rutas y mapas indoor."
    echo "===================================================="
    echo "✅ ¡HECHO! Versión con Importador lista."
    echo "===================================================="
else
