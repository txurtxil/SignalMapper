...
def get_outdoor_content(page: ft.Page, lang: str):
    ...
    def update_ui_map(lat: float, lon: float, metodo: str):
        ...
        map_image.src = (
            ...
            f"?center={lat},{lon}&zoom=16&size=320x300"
    ...
    def on_position(e):
        try:
            update_ui_map(e.coords.latitude, e.coords.longitude, "GPS Satélite")
        except AttributeError:
            update_ui_map(e.latitude, e.longitude, "GPS Satélite")
    ...
    if not hasattr(page, "outdoor_geolocator"):
        try:
            ...
            status_text.value = "❌ Error: flet-geolocator no instalado"
            status_text.color = ft.Colors.RED
    ...
    def handle_click(e):
        ...
        async def task():
            try:
                ...
                if geo:
                    await geo.request_permission_async()
                    await geo.get_current_position_async()
                    for _ in range(10):
                        await ft.asyncio.sleep(0.5)
                        if "GPS Satélite" in (status_text.value or ""):
                            return
                    get_ip_location()
                else:
                    get_ip_location()
